...
TVGuide::Sql - (MySql) database access
...
sub init_sql
{
	...
	my $RECMASK = $TVGuide::Base::Constants::REC_MASK ;
	my $REC_NONE = TVGuide::Base::Constants::record_types_lookup('none') ;
	my $REC_ONCE = TVGuide::Base::Constants::record_types_lookup('once') ;
...
sub dayofweek
{
	...
	return $SQL_DOW{TVGuide::Time::dt2dayname($dt)} ;
}
...
$this->sql->debug($this->debug) ;
	...
	TVGuide::Base::DbgTrace::startfn('insert_recording') ;
...
print " + rid=$rid\n" if $this->debug ;
	...
	TVGuide::Base::DbgTrace::add_rec($rec_href, "inserted - new rid $rid") ;
$this->sql->debug(0) ;
...
$this->sql->debug($this->debug) ;
	...
	TVGuide::Base::DbgTrace::startfn('update_recording') ;
...
$this->sql->debug(0) ;
	...
	TVGuide::Base::DbgTrace::add_rec($rec_href, 'updated') ;
}
...
$this->sql->debug($this->debug) ;
	...
	# Get sql query name
	my $query_name = TVGuide::Base::Constants::query_name($record_type) ;
...
$this->sql->debug($this->debug) ;
	...
	TVGuide::Base::DbgTrace::startfn('update_schedule_table') ;
...
$this->sql->debug($this->debug) ;
	...
	TVGuide::Base::DbgTrace::startfn('update_iplay_table') ;
...
$this->sql->debug($this->debug) ;
	...
	TVGuide::Base::DbgTrace::startfn('insert_recorded') ;
...
$this->sql->debug($this->debug) if $this->debug >= 2 ;
	...
	TVGuide::Base::DbgTrace::startfn('delete_recorded') ;
...
$this->sql->debug($this->debug) if $this->debug >= 2 ;
	...
	TVGuide::Base::DbgTrace::startfn('select_recorded') ;
...
$this->sql->debug($this->debug) if $this->debug >= 2 ;
	...
	TVGuide::Base::DbgTrace::startfn('update_recorded') ;
