package TVGuide::Crontab ;

=head1 NAME

TVGuide::Crontab - crontab utils
...
BEGIN {
	...
	$opts_href = {
		...
		'padding'		=> 2,
		'early'			=> 30,
		'recprog'		=> 'dvbt-ffrecord',
		'iplayprog'		=> 'get_iplayer',
	...
	my $today_dt = TVGuide::Time::parse_date("today", "0:00") ;
	$today_mins = TVGuide::Time::dt2mins($today_dt) ;
...
sub create_blocks
{
	...
	TVGuide::Base::DbgTrace::startfn('Crontab::create_blocks') ;
...
#print "pad=$opts_href->{padding}\n" if $debug>=3 ;
	...
	{
		my @sorted = sort { TVGuide::Prog::start_cmp($a, $b) } @{$db{$adap}} ;
...
#$opts_href->{'app'}->prt_data("Sorted progs:", \@sorted) if $debug>=2  ;
		...
		for(my $i=0; $i<=$last_entry; ++$i)
		{
			...
			if ($i==0)
			{
				...
				{
					TVGuide::Prog::pad($prog_href, 'start', $PAD) ;
				}
			}
			else
			{
				...
				foreach my $prog_href (@progs)
				{
					if (TVGuide::Prog::check_pad($sorted[$i-1], $prog_href, $PAD))
					{
						TVGuide::Prog::pad($prog_href, 'start', $PAD) ;
					}
				}				
		...
		for(my $i=0; $i<=$last_entry; ++$i)
		{
			...
			if ($i==$last_entry)
			{
				...
				foreach my $prog_href (@progs)
				{
					TVGuide::Prog::pad($prog_href, 'end', $PAD) ;
				}
			}
			else
			{
				...
				foreach my $prog_href (@progs)
				{
					my $diff_secs = TVGuide::Time::timediff_secs($prog_href, $sorted[$i+1]) ;
					...
					else
					{
						...
						TVGuide::Prog::finish_early($prog_href, $opts_href->{'early'}) ;
					}
				}
			}
		}
		...
		for(my $i=0; $i<=$last_entry; ++$i)
		{
			...
			if ($i==$last_entry)
			{
				...
				foreach my $prog_href (@progs)
				{
					TVGuide::Prog::timeslip($prog_href, $MAX_TIMESLIP) ;
				}
			}
			else
			{
				...
				{
					# start by clearing
					TVGuide::Prog::timeslip($prog_href, 0) ;
					
					# check space
					my $diff_secs = TVGuide::Time::timediff_secs($prog_href, $sorted[$i+1]) ;
#print "\nTIMESLIP: comparing :\n" ;
#print "  this: " ; TVGuide::Prog::disp_sched_entry($prog_href) ;				
#print "  next: " ; TVGuide::Prog::disp_sched_entry($sorted[$i+1]) ;				
#print "    diff secs=$diff_secs\n" ; 				
					...
					if ($MAX_TIMESLIP)
					{
						...
						elsif ($diff_secs > $PAD_SECS)
						{
							...
							TVGuide::Prog::timeslip($prog_href, $diff_secs / 60) ;
						}
					}
#print "  final: " ; TVGuide::Prog::disp_sched_entry($prog_href) ;				
...
sub create_iplay_blocks
{
	...
	TVGuide::Base::DbgTrace::startfn('Crontab::create_iplay_blocks') ;

	my @sorted = sort { TVGuide::Prog::start_cmp($a, $b) } @{$db_aref} ;
...
sub create_block
{
	...
	my $fname = TVGuide::Series::get_filename($entry_href) ;
	...
	if ($type eq 'radio')
	{
		...
		my $episode = TVGuide::Path::episode_string($entry_href) ;
...
#	}
#	$dir = TVGuide::Path::cleanpath($dir) ;
...
#my $date = TVGuide::Time::dt_format($entry_href->{'start_datetime'}, "%Y%m%d%H%M") ;			 
#$cmd .= " >> $opts_href->{log_dir}/dvbt-multirec-DVB${dvb}-${date}.log 2>&1" ;
	$cmd .= " >> $opts_href->{log_dir}/dvbt-record.log 2>&1" ;
		
	# Set time
	my $datetime = TVGuide::Time::dt_format($entry_href->{'start_datetime'}, "%M %H %d %m *") ;			 
...
sub create_iplay_block
{
	...
	my ($fdir, $file) = TVGuide::Series::get_filename($entry_href) ;
...
#	$dir = TVGuide::Path::expand_path($dir) ;
	...
	my $iplay_ctrl_file = TVGuide::Time::dt_format($entry_href->{'start_datetime'}, "iplay-%Y-%m-%d".$opts_href->{'run_ext'}) ;
	$iplay_ctrl_file = TVGuide::Path::cleanpath("$iplay_ctrl_file") ;
...
sub create_mux_block
{
	...
	my $start_dt_mins = TVGuide::Time::dt2mins($dt) ;
	...
	$rec_file = TVGuide::Path::cleanpath("$rec_file") ;
...
$cmd .= " -dbg-sql 1";	
	...
	foreach my $entry_href (@{$mux_href->{'multiplex'}})
	{
		...
		if ($idx)
		{
			...
			my $entry_start_dt_mins = TVGuide::Time::dt2mins($dt) ;
		...
		$block_comment .= " (" . TVGuide::Time::dt2hms($dt) . " - " . TVGuide::Time::dt2hms($end_dt) . " Duration: " . TVGuide::Time::secs2time($entry_href->{duration_secs}) . ")" ;
...
#		my $recfilename = "$dir/$fname.$ext" ;
	...
	# Set time
	my $datetime = TVGuide::Time::dt_format($mux_href->{'start_datetime'}, "%M %H %d %m *") ;			 

	# Log
	##$cmd .= " >> $opts_href->{log} 2>&1" ;
#my $dvb = $mux_href->{'adapter'} ;
#my $date = TVGuide::Time::dt_format($mux_href->{'start_datetime'}, "%Y%m%d%H%M") ;			 
#$cmd .= " >> $opts_href->{log_dir}/dvbt-multirec-DVB${dvb}-${date}.log 2>&1" ;
	$cmd .= " >> $opts_href->{log_dir}/dvbt-record.log 2>&1" ;
...
print " + got (chan=$chan, prog=$prog, secs=$duration_secs)\n" if $debug ;
			...
			my $dt = TVGuide::Time::parse_date($date, $time) ;
			my $dt_mins = TVGuide::Time::dt2mins($dt) ;

			my $dt_end = TVGuide::Time::dt_offset($dt, "+ $duration_secs seconds") ;
			my $end_time = TVGuide::Time::dt2hms($dt_end) ;
