package TVGuide::Recording ;

=head1 NAME

TVGuide::Recording - methods for processing the initial recording requests
...
my %FIELDS = (
	...
	'tvsql'		=> undef,		# An initialised TVGuide::Sql object
	'channels'	=> {},
...
sub new
{
	...
	my $today_dt = TVGuide::Time::today_dt() ;
	my $today_mins = TVGuide::Time::dt2mins($today_dt) ;
	$this->today_mins($today_mins) ;
	my $today = TVGuide::Time::dt2date($today_dt) ;
	$this->today($today) ;

#print "TVGuide::Recording - today $today_dt  today_mins $today_mins\n" ;
	
	my $tommorrow_dt = TVGuide::Time::dt_offset($today_dt, "+ 1 day") ;
	my $tommorrow = TVGuide::Time::dt2date($tommorrow_dt) ;
...
sub get_recording
{
	...
	my @recording = $this->tvsql->select_dvbt_recording() ;
		...
		TVGuide::Base::DbgProf::waypoint("read database") ;
	
	## Sort by date & priority etc
	@recording = sort { TVGuide::Prog::start_cmp($a, $b) } @recording ;
	
	## Set up times
	$this->fix_recording_times(\@recording) ;
	
		TVGuide::Base::DbgProf::waypoint("fixed times") ;

#	## Set up pathspecs
#	$this->fix_pathspecs(\@recording) ;
#	
#		TVGuide::Base::DbgProf::waypoint("fixed pathspecs") ;

	## Set up channel types (tv/radio)
	$this->fix_chan_types(\@recording) ;
	
		TVGuide::Base::DbgProf::waypoint("fixed chan_types") ;
		TVGuide::Base::DbgTrace::trace_init(\@recording) ;
	...
	@recording = $this->process_recording(\@recording, {'DVBT' => 1}) ;
		...
		TVGuide::Base::DbgProf::waypoint("processed recordings") ;
		TVGuide::Prog::disp_sched("recording list before filter=", \@recording) if $this->debug ;

	## Filter out old events
	@recording = $this->filter_recording($start_date, \@recording) ;

		TVGuide::Prog::disp_sched("filtered recordings=", \@recording) if $this->debug ;
		TVGuide::Base::DbgProf::waypoint("filtered recordings") ;
...
print "#== get_recording_from_spec() ==\n" if $this->debug ;
	...
	my @recording = () ;
	my $record_group = TVGuide::Base::Constants::record_group($recspec_href->{'rec'}) ;
...
$this->prt_data("Found existing rec_href=", $rec_href) if $this->debug ;
	...
	$this->prt_data("program search=", \@listings) if $this->debug >= 2 ;
				if (@listings)
				{
					...
					my $record_id = $TVGuide::Base::Constants::NEW_RID ;
					my $priority = $recspec_href->{'pri'} || $TVGuide::Base::Constants::DEFAULT_PRIORITY ;
	
	print " + create new (pri=$priority)\n" if $this->debug ;
	
					$rec_href = TVGuide::Prog::new_recording($listing_href, $recspec_href->{'rec'}, $record_id, $priority) ;
				}
			}
		}
...
#	foreach (@$recording_aref)
#	{
#		$_->{'pathspec'} ||= TVGuide::Series::default_pathspec($_) ;
#	}
#}
...
TVGuide::Prog::disp_sched("existing recording list=", $recording_aref) if $this->debug ;

$this->prt_data("recordings=", $recording_aref) if $this->debug>=10 ;	

	TVGuide::Base::DbgTrace::startfn('process_recording') ;
	...
	foreach my $rec_href (sort {$b->{'priority'} <=> $a->{'priority'}} @$recording_aref)
	{
		...
		{
			next unless TVGuide::Base::Constants::has_dvbt($record) ;
		}
		if ($options_href->{'IPLAY'})
		{
			next unless TVGuide::Base::Constants::has_iplay($record) ;
		}
		
		my $record_group = TVGuide::Base::Constants::record_group($record) ;
		...
		foreach my $listing_href (@listings)
		{
			...
			my $new_sched_href = TVGuide::Prog::new_recording($listing_href, $record, $record_id, $priority) ;
...
print " + ADD NEW PID : $new_sched_href->{'pid'}\n" if $this->debug ;	
				...
				TVGuide::Base::DbgTrace::add_rec($new_sched_href, "added new : PID $new_sched_href->{'pid'}") ;
...
TVGuide::Prog::disp_sched("recording list=", \@recording) if $this->debug ;
...
TVGuide::Prog::disp_sched("recording list (fixed times/types)=", \@recording) if $this->debug ;
...
sub _set_fuzzy_entry
{
	...
	$rec_href->{'pid'} = $TVGuide::Base::Constants::FUZZY_PID ;
	$rec_href->{'start'} = $TVGuide::Base::Constants::FUZZY_TIME ;
	$rec_href->{'duration'} = $TVGuide::Base::Constants::FUZZY_DURATION ;
	$rec_href->{'tva_series'} = '-' ;
	$rec_href->{'tva_prog'} = '-' ;
}
...
sub _create_fuzzy_entry
{
	...
	my $rec_href = TVGuide::Prog::new_recording($listing_href, $record, $record_id, $priority) ;
...
$this->prt_data("recordings=", $recording_aref) if $this->debug>=10 ;	
	...
	TVGuide::Base::DbgTrace::startfn('process_iplay_recording') ;
	...
	foreach my $href (@$recording_aref)
	{
		...
		my @listings ;
		my $record_type = TVGuide::Base::Constants::record_base($rec_href->{'record'}) ;
		my $record_group = TVGuide::Base::Constants::record_group($rec_href->{'record'}) ;
		my $record_id = $rec_href->{'rid'} ;
		my $priority = $rec_href->{'priority'} ;
...
$this->prt_data("rec_href=", $rec_href) if $this->debug>=4 ;	
		...
		TVGuide::Base::DbgTrace::add_rec($rec_href, 'seen') ;
		
		# Check record type
		if ($record_type >= TVGuide::Base::Constants::record_types_lookup('multi'))
		{
			...
			if (@real_listings)
			{
				...
				TVGuide::Base::DbgTrace::add_rec($rec_href, 'iplay seen this real prog') ;
$this->prt_data(" + using real : rec_href=", $rec_href) if $this->debug>=4 ;	
	...
	print " + + * * adding new IPLAY entry for $date : new pid=$new_href->{'pid'} [old $rec_href->{'pid'}]\n" if $this->debug ;	
					...
					TVGuide::Base::DbgTrace::add_rec($rec_href, 'use existing real program') ;
	...
	print " + + + adjusted pid\n" if $this->debug ;	
						...
						TVGuide::Base::DbgTrace::add_rec($rec_href, "created dummy based on $new_href->{'prog_pid'}") ;
...
TVGuide::Prog::disp_sched("existing recording list=", $recording_aref) if $this->debug ;
TVGuide::Prog::disp_sched("new recording list=", \@recording) if $this->debug ;
...
sub _process_recording
{
	...
	my $start_dt = TVGuide::Time::parse_date($rec_href->{date}, $rec_href->{start}) ;
	my $start_mins = TVGuide::Time::time2mins($rec_href->{start}) ;
...
print " + checking entry RID $record_id ($rec_href->{title} @ $rec_href->{date} $rec_href->{start}) - type=$record_type rec=$record [$record_group:$record_type] pri=$priority\n" if $this->debug ;	
	...
	if ($record_type == TVGuide::Base::Constants::record_types_lookup('once'))
	{
		my $start_dt_mins = TVGuide::Time::dt2mins($start_dt) ;
...
print " + Start ($rec_href->{date} $rec_href->{start} => $start_dt) mins=$start_dt_mins, Today mins=".$this->today_mins."\n" if $this->debug ;	
		...
		if ( ($start_dt_mins >= $this->today_mins) || TVGuide::Base::Constants::has_iplay($record) )
		{
			# Once - ensure this program hasn't changed
			@listings = $this->tvsql->select_program($rec_href) ;
	
			TVGuide::Base::DbgTrace::add_rec($rec_href, 'search : single') ;
		}
		else
		{
			...
			TVGuide::Base::DbgTrace::add_rec($rec_href, 'skip old : single') ;
		}
	}
...
print " + + start_dt=$start_dt, start_mins=$start_mins, margin_mins=$margin_mins\n" if $this->debug>=10 ;	
			...
			{
				$rec_href->{'start_min'} = TVGuide::Time::mins2time($start_mins - $margin_mins) ;
print " + + + Set min=$rec_href->{'start_min'}\n" if $this->debug>=10 ;	
			}
			$rec_href->{'start_max'} = "23:59" ;
			if (($start_mins + $margin_mins) < $TVGuide::Base::Constants::DAY_MINS )
			{
				$rec_href->{'start_max'} = TVGuide::Time::mins2time($start_mins + $margin_mins) ;
print " + + + Set max=$rec_href->{'start_max'}\n" if $this->debug>=10 ;	
			}
			
			$rec_href->{'dayofweek'} = TVGuide::Sql->dayofweek($start_dt) ; 
			
			## Check for series record
			if ($record_type == TVGuide::Base::Constants::record_types_lookup('series'))
			{
				# if no series specified, then fall back to recording all on this channel
				if (!$rec_href->{'tva_series'} || ($rec_href->{'tva_series'} eq '-'))
				{
					$record_type = TVGuide::Base::Constants::record_types_lookup('multi') ;
...
print " + Search title=$rec_href->{'title'}\n" if $this->debug>=10 ;	
	...
	$this->prt_data("windowed listings=", \@listings) if $this->debug>=10 ;	
			...
			TVGuide::Base::DbgTrace::add_rec($rec_href, {
...
print "#== filter_recording() ==\n" if $this->debug ;	
	...
	TVGuide::Base::DbgTrace::startfn('filter_recording') ;

	# Get now to compare with
	my $now = TVGuide::Time::parse_date($start_date) ;
	my $now_mins = TVGuide::Time::dt2mins($now) ;
...
print " + + Added $rec_href->{'title'} @ $rec_href->{'date'} $rec_href->{'start'}\n" if $this->debug ;
			...
			TVGuide::Base::DbgTrace::add_rec($rec_href, '** Accepted **') ;
		}
		else
		{
print " + + missed start/end\n" if $this->debug ;
			TVGuide::Base::DbgTrace::add_rec($rec_href, 'missed start/end') ;
		}
	}
...
sub _fix_iplay_date
{
	...
	TVGuide::Prog::set_times($rec_href) ;
...
print "#== fix_iplay_date() ==\n" if $this->debug ;	
	...
	TVGuide::Base::DbgTrace::startfn('fix_iplay_date') ;
	
	my $RECORD_ONCE = TVGuide::Base::Constants::record_types_lookup('once') ;
	my $TODAY_MINS = $this->today_mins ;
	
	my $record = $new_rec_href->{'record'} ;
	my $record_type = TVGuide::Base::Constants::record_base($new_rec_href->{'record'}) ;
	
	## See if one-off IPLAY recording
	my $needs_fixing = 0 ;
	if (TVGuide::Base::Constants::has_iplay($record) && ($record_type == $RECORD_ONCE) )
	{
print " + + IPLAY : one-off\n" if $this->debug ;

		## Old recording
		my $start_dt = TVGuide::Time::parse_date($new_rec_href->{date}, $new_rec_href->{start}) ;
		my $start_dt_mins = TVGuide::Time::dt2mins($start_dt) ;
...
#
#print "#== fix_iplay_pid() ==\n" if $this->debug ;	
#TVGuide::Prog::disp_sched(" + before fix_iplay_pid=", $recording_aref) if $this->debug >= 2 ;
#
#	foreach my $rec_href (@$recording_aref)
#	{
...
#	}
#
#TVGuide::Prog::disp_sched(" + after fix_iplay_pid=", $recording_aref) if $this->debug >= 2 ;
#}
