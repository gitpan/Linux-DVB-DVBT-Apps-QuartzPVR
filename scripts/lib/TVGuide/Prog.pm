package TVGuide::Prog ;

=head1 NAME

TVGuide::Prog - Program methods
...
sub set_times
{
	...
	$entry_href->{'date'} = TVGuide::Time::reformat_date($entry_href->{'date'}) ; 
	
	# get duration
	my $duration_mins = TVGuide::Time::time2mins($entry_href->{'duration'}) ;
	my $duration_add = sprintf "+ %dmin", $duration_mins ;
	$entry_href->{'duration_secs'} = 60 * $duration_mins ;
	
	# cache the start & end date/times
	my $dt = TVGuide::Time::parse_date($entry_href->{date}, $entry_href->{start}) ;
	$entry_href->{'start_datetime'} = $dt ;
	$entry_href->{'start_dt_mins'} = TVGuide::Time::dt2mins($dt) ;
	
	# add duration to start to get end
	$dt = TVGuide::Time::dt_offset($dt, $duration_add) ;
	$entry_href->{'end_datetime'} = $dt ;
	$entry_href->{'end'} = TVGuide::Time::dt2hms($dt) ;
	$entry_href->{'end_date'} = TVGuide::Time::dt2date($dt) ;
	$entry_href->{'end_dt_mins'} = TVGuide::Time::dt2mins($dt) ;
...
print "check_pad(end=$end_entry->{date} @ $end_entry->{start}-$end_entry->{end} - start=$start_entry->{date} @ $start_entry->{start}-$start_entry->{end})\n" if $debug>=4 ;
	...
	# diff
	my $diff_secs = TVGuide::Time::timediff_secs($end_entry, $start_entry) ;
...
print " + start : $entry_href->{start_datetime} - $entry_href->{end_datetime} (duration: $entry_href->{duration_secs} secs)\n" ;
	...
	if ($pad) 
	{
		...
		{
			## start early
			$entry_href->{'start_datetime'} = TVGuide::Time::dt_offset($entry_href->{'start_datetime'}, $pad_sub) ;
			$entry_href->{'start_dt_mins'} = TVGuide::Time::dt2mins($entry_href->{'start_datetime'}) ;
		}
		else
		{
			## end late
			$entry_href->{'end_datetime'} = TVGuide::Time::dt_offset($entry_href->{'end_datetime'}, $pad_add) ;
			$entry_href->{'end_dt_mins'} = TVGuide::Time::dt2mins($entry_href->{'end_datetime'}) ;
		}
...
sub timeslip
{
	...
	if ($timeslip > 0) 
	{
		$entry_href->{'end_datetime'} = TVGuide::Time::dt_offset($entry_href->{'end_datetime'}, "+ ${timeslip}min") ;
		$entry_href->{'end_dt_mins'} = TVGuide::Time::dt2mins($entry_href->{'end_datetime'}) ;
...
sub finish_early
{
	...
	if ($early_secs) 
	{
		...
		## end
		$entry_href->{'end_datetime'} = TVGuide::Time::dt_offset($entry_href->{'end_datetime'}, $pad_sub) ;
...
print "new_recording(title=$listing_href->{'title'}, episode=$listing_href->{'episode'}, num=$listing_href->{'num_episodes'}, subtitle=$listing_href->{'subtitle'})\n" if $debug ;
	...
	$record = TVGuide::Base::Constants::record_types_lookup('once') unless defined($record) ;
...
sub disp_sched_entry
{
	...
	print "(".TVGuide::Time::dt2hms($prog_href->{'start_datetime'}) . " - " . TVGuide::Time::dt2hms($prog_href->{'end_datetime'}).") " ;	
