package TVGuide::Report ;

=head1 NAME

TVGuide::Report - Write report of new scheduling
...
sub report_prog_entry
{
	...
	$text .= "[$prog_href->{record}::"
		.TVGuide::Base::Constants::record_group($prog_href->{record})
		.":"
		.TVGuide::Base::Constants::record_types($prog_href->{record})
		."] " ;
	...
	if ($EXTRA_INFO)
	{
		...
		$text .= "< " . TVGuide::Time::dt2hms($prog_href->{'start_datetime'}) . " - " . TVGuide::Time::dt2hms($prog_href->{'end_datetime'}) . " > " ;
	}
...
sub report_prog_list
{
	...
	my @sorted = sort { TVGuide::Prog::start_cmp($a, $b) } @$schedule_aref ;
...
#my $prev_end_dt ;
	...
	foreach my $prog_href (@sorted)
	{
		if ($EXTRA_INFO)
		{
#print "\nGAP? prog=".TVGuide::Time::dt2date($prog_href->{'start_datetime'})." - ".TVGuide::Time::dt2date($prog_href->{'end_datetime'})."\n" ;
			if ($prev_end_dt_mins)
			{
				my $start_dt_mins = TVGuide::Time::dt2mins($prog_href->{'start_datetime'}) ;
#print "GAP? prev=".TVGuide::Time::dt2hms($prev_end_dt)." ($prev_end_dt_mins), start=".TVGuide::Time::dt2hms($prog_href->{'start_datetime'})." ($start_dt_mins), diff=".($prev_end_dt_mins - $start_dt_mins)."\n" ;
...
#print " + GAP FOUND\n" ;
			...
			$prev_end_dt_mins = TVGuide::Time::dt2mins($prog_href->{'end_datetime'}) ;
...
RECSPECLINES
	...
	## cron
	my $cron_lines = TVGuide::Crontab::block_lines() ;
...
REPORT
	...
	if (TVGuide::Base::DbgTrace::trace_flag() )
	{
		$report .= "DEBUG TRACE\n" ;
		$report .= "===========\n\n" ;
			
		## Print trace (if required)
		$report .= TVGuide::Base::DbgTrace::create_trace_report() ;
	}
